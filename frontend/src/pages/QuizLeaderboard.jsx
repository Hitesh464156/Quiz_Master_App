  ...
  const fetchResult = async (page = 1) => {
    try {
      ...
      const { data } = await axios.get(
        `${config.backendURL}/api/v1/quiz/leaderboard/${id}?page=${page}`,
        axiosConfig
      );
        ...
        {!leaderboard ? (
          ...
          <div className="w-full max-w-md">
            <div className="w-full flex justify-between items-center gap-5 mb-6">
              <button
                className={`h-10 w-24 rounded-lg font-semibold cursor-pointer ${
                  isLocked
            ...
            <table className="w-full bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-950 overflow-hidden">
              ...
              <tbody>
                {leaderboard.map((item, index) => (
                  <tr
                    key={index}
                    className="border-b hover:bg-gray-100 dark:hover:bg-gray-700 transition duration-200"
                  >
                    <td className="py-2 px-4 text-gray-600 dark:text-gray-200">
                      {item.name}
                    </td>
                    <td className="py-2 px-4 text-gray-600 dark:text-gray-200">
                      {item.totalScore}
