...
const CreateQuizForm = () => {
  ...
  const handleChangeAnswer = (index) => {
    ...
    if (answers.includes(index)) {
      updatedQuestions[currentQuestionIndex].answers = answers.filter(
        (answer) => answer !== index
      );
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${config.backendURL}/api/v1/quiz/create`,
  ...
  return (
    ...
    >
      <div className="w-full flex flex-col gap-3">
        <label
          htmlFor="title"
          className="text-xl max-sm:text-lg text-gray-700 dark:text-gray-200"
        >
          Title
        </label>
        <input
      ...
      <div className="w-full flex flex-col gap-3">
        ...
        <textarea
          ...
          name="question"
          id="question"
          className="w-full p-3 bg-gray-200 dark:bg-gray-700 rounded-md text-lg max-sm:text-base border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-400 dark:focus:ring-blue-500"
          required
      ...
      <div className="w-full flex flex-col gap-3">
        <label className="text-xl max-sm:text-lg text-gray-700 dark:text-gray-200">
          Options
        </label>
      ...
      <div className="w-full flex items-center justify-between gap-4">
        <button
          type="button"
          onClick={handlePrev}
          className="h-12 w-24 bg-gray-600 dark:bg-gray-400 hover:bg-gray-700 dark:hover:bg-gray-300 rounded-md text-xl max-sm:text-lg text-white dark:text-black disabled:opacity-50"
      ...
      <div className="w-full flex items-center justify-center gap-4">
        {currentQuestionIndex === questions.length - 1 ? (
          <button
            type="button"
            className="h-12 w-40 bg-blue-600 dark:bg-blue-400 hover:bg-blue-700 dark:hover:bg-blue-300 rounded-md text-xl max-sm:text-lg text-white dark:text-black"
          ...
          <button
            type="button"
            className="h-12 w-40 bg-orange-600 dark:bg-orange-400 hover:bg-orange-700 dark:hover:bg-orange-300 rounded-md text-xl max-sm:text-lg text-white dark:text-black"
      ...
      <div className="w-full flex items-center justify-center gap-4">
        <button
          ...
          className={`h-12 w-40 ${
            loading
              ...
              : "bg-green-600 dark:bg-green-400 hover:bg-green-700 dark:hover:bg-green-300 text-white dark:text-black"
          } rounded-md text-xl max-sm:text-lg`}
        >
          {loading ? (
            <ImSpinner2
              size={22}
        ...
        <button
          type="button"
          onClick={() => navigate(-1)}
          className="h-12 w-40 bg-red-600 dark:bg-red-400 hover:bg-red-700 dark:hover:bg-red-300 rounded-md text-xl max-sm:text-lg text-white dark:text-black"
        >
          Cancel
        </button>
      </div>
