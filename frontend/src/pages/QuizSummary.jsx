  ...
  const fetchResult = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${config.backendURL}/api/v1/quiz/summary/${id}`,
        axiosConfig
      );
      setAverageRating(data.averageRating);
      setStarCounts(data.starCounts);
      setFeedbackMessages(
        data.feedbackMessages.filter(
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= rating) {
        ...
        stars.push(
          <FaStarHalfAlt key={i} size={30} className="text-yellow-500" />
        );
      } else {
    ...
    for (let i = 0; i < 5; i++) {
      stars.push(
        <FaStar
          key={i}
          size={18}
          className={`${i < count ? "text-yellow-500" : "text-gray-300"} mr-1`}
        />
      );
    }
    return stars;
      ...
      <div className="w-[90%] max-w-4xl min-h-[600px] mb-5 p-16 max-sm:p-8 bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-gray-950 flex flex-col items-center justify-center text-gray-700 dark:text-gray-200">
        <div className="w-full max-w-2xl">
          ...
          <div className="w-full mb-6">
            ...
            {!averageRating ? (
              <TbFidgetSpinner className="mx-auto animate-spin" size={40} />
            ) : (
