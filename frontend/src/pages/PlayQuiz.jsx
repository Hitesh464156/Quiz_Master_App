...
const PlayQuiz = () => {
  ...
  useEffect(() => {
    ...
    setTimeRemaining(
      savedTime ? parseInt(savedTime, 10) : location.state.quiz.duration * 60
    );
    const savedAttemptedQuestions = JSON.parse(
  ...
  useEffect(() => {
    const currentQuestion = questions.find(
      (q) => q.questionId === quiz.questions[questionNumber]._id
    );
    if (currentQuestion) {
  ...
  const handleClearSelection = () => {
    const updatedQuestions = questions.filter(
      (question) => question.questionId !== quiz.questions[questionNumber]._id
    );
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${config.backendURL}/api/v1/quiz/attempt`,
        { quizId: quiz._id, questions },
        axiosConfig
      );
      setQuestionNumber(0);
      setSelectedOptions([]);
  ...
  const isSubmittedAnswer = (questionId, index) => {
    return questions.some(
      (q) => q.questionId === questionId && q.answers.includes(index)
    );
  };
          ...
          <div className="w-full flex flex-col justify-between">
            ...
            <div className="w-full flex flex-col justify-center gap-4 mb-6">
              {quiz.questions[questionNumber].options.map((option, index) => (
                <div
                  key={index}
                  className={`p-4 rounded-md shadow-md dark:shadow-gray-900 flex items-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${
                    selectedOptions.includes(index)
                      ? isSubmittedAnswer(
                          quiz.questions[questionNumber]._id,
                          index
                        )
