  ...
  const fetchReviewResult = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${config.backendURL}/api/v1/quiz/review-result/${id}`,
        axiosConfig
      );
        ...
        {result ? (
          <div className="w-full flex flex-col justify-between">
            ...
            <div className="w-full flex flex-col justify-center gap-4 mb-6">
              {result.questions[questionNumber].questionId.options.map(
                (option, index) => (
                  <div
                    key={index}
                    className={`p-4 rounded-md shadow-md dark:shadow-gray-900 ${getOptionStyle(
                      index,
                      result.questions[questionNumber].questionId.answers,
                      result.questions[questionNumber].answers
                    )}`}
                  >
                    <input
                      type="checkbox"
